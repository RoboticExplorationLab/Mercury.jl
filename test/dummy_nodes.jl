...
mutable struct PubNode <: Hg.Node
    ...
    function PubNode(test_pub_ip::String, test_pub_port::String, rate::Float64, debug::Bool)
...
mutable struct SubNode <: Hg.Node
    ...
    function SubNode(test_sub_ip::String, test_sub_port::String, rate::Float64, debug::Bool)
        # Adding the Ground Vicon Subscriber to the Node
        pubNodeIO = Hg.NodeIO()
        rate = rate
        should_finish = false
...
function main(; rate = 100.0, debug = false)
    ...
    pub_node = PubNode(test_ip, test_port, rate, debug)

    sub_node = SubNode(test_ip, test_port, rate, debug)
